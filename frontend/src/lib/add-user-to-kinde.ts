...
async function getAuthToken(): Promise<Token> {
  try {
    const response: FetchResponse = await fetch(`${process.env.KINDE_ISSUER_URL}/oauth2/token`, {
      method: "POST",
      headers: {
...
async function addLogoutUrlToKinde(token: Token): Promise<void> {
  try {
...
async function addCallbackUrlToKinde(token: Token): Promise<void> {
  try {
    const response: FetchResponse = await fetch(
